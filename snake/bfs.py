...
class Bfs:
	def get_next(self, x1, y1, x2, y2, cells):
		s, f = top(x1, y1), top(x2, y2)
		p = self.search(30, 30, s, f, cells)
	...
	def search(self, N, M, start, finish, cells):
	    ...
	    while queue:
	        ...
	        for direction in dx:
		        ...
		        if u == finish or u.x < N and u.x >=0 and u.y < N and u.y >= 0 and not used[u.x][u.y] and cells[u.x][u.y] is None:
		        	used[u.x][u.y] = True
		        	p[u.x][u.y] = v
		        	queue.append(u)
		        	if u == finish:

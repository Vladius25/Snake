...
Мы находимся в состояния поиска лучшего формата для проведения параллели, а пока расскажем как это было в том году и планируем в этом.
...
Как и в том году мы расскажем вам про unit-тестирование, проектирование, чистый код, TDD, git и другие вещи, которые не только встречаются каждый день в разработке, но и очень интересны сами по себе.
...
Кроме того мы попробуем парное программирование, посоревнуемся в написании ботов, поиграем в code-retreat.
...
Мы верим, что те, кто прошли параллель B' или выше в ЛКЛ или ЛКШ, обладают всем вышеперечисленным выше (ну, может кроме знания Python).
Однако при поступлении будет учитываться в основном вступительная работа.
...
**Способ 1** (предпочтиельный): Склонировать проект по SSH или HTTPS с основной вкладки проекта.

**Способ 2**: (TODO: устарело =()Перейти с данной страницы на вкладку Repository, щелкнуть по значку справа и загрузить архив с проектом в любом из предложенных форматов
...
На сайте sicamp.ru можно будет прикрепить архив/ссылку на репозиторий с выполненной работой.
Не забудьте заполнить тематическую анкету для поступающих в P.

Срок выполнения работы - 27 мая, 23:59 по московскому времени.
